...
Now, let's try to get a better sense at where that money is going, and how much has been spent. I want to narrow in on the sub-recipients.=-
...
Finally, let's narrow in on our "Other" recipient type. Can we learn a little bit about where that spending went, if it wasn't directly to states, counties, municipalities, or traibal governments?
...
Other <- data %>%
  ...
  select(SUB_Distinct, Sub.award.amount, State, Type, Award.type)%>%
   mutate(Sub_type = if_else(str_detect(SUB_Distinct,"School|EDUCATION|ELEMENTARY|ACADEM"),"Educational",if_else(str_detect(SUB_Distinct,"BANK"),"Bank","Other")))

Other

Other%>%
   ggplot()+
     geom_bar(mapping= aes(x = Sub_type))
